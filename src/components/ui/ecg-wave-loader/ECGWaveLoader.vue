<template>
    ...
    >
      <defs>
        <clipPath :id="clipId">
          <rect :x="0" :y="0" :width="width" :height="height" :rx="rounded ? radius : 0" :ry="rounded ? radius : 0" />
        </clipPath>
  ...
  const pathD = computed(() => generateECGPath(props.width, props.height, cycles))
  ...
  function generateECGPath(w, h, nCycles = 3) {
    ...
    for (let i = 0; i < nCycles; i++) {
      const x0 = i * seg
      const x1 = x0 + seg * 0.10 // baseline lead-in
      ...
      d += ` C ${x0 + seg * 0.12} ${yMid}, ${x0 + seg * 0.16} ${yMid - amp * 0.15}, ${x2} ${yMid - amp * 0.20}`
      ...
      // quick return
      d += ` L ${x5} ${yMid + under * 0.20}`
  
      // recovery hump (smooth curve back to baseline)
      d += ` C ${x5 + seg * 0.05} ${yMid - amp * 0.10}, ${x6 - seg * 0.05} ${yMid - amp * 0.12}, ${x6} ${yMid - amp * 0.08}`
